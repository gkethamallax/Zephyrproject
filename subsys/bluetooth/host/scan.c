...
K_MUTEX_DEFINE(scan_update_mutex);
K_MUTEX_DEFINE(scan_explicit_params_mutex);
...
static void select_scan_params(struct bt_le_scan_param *scan_param)
{
	...
	/* 3. Priority: choose custom parameters */
		...
		*scan_param = (struct bt_le_scan_param){
		...
		if (atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_SYNC_SYNCING) ||
		    atomic_test_bit(scan_state.scan_flags, BT_LE_SCAN_USER_SCAN_BEFORE_INITIATE)) {
...
static int scan_update(void)
{
	...
	k_mutex_lock(&scan_update_mutex, K_FOREVER);
	...
	case SCAN_ACTION_STOP:
		...
		if (err) {
			LOG_DBG("Could not stop scanner.");
			break;
		}
	...
	case SCAN_ACTION_UPDATE:
		err = cmd_le_set_scan_enable(BT_HCI_LE_SCAN_DISABLE,
					     BT_HCI_LE_SCAN_FILTER_DUP_DISABLE);
		if (err) {
			LOG_DBG("Could not stop scanner to update.");
			break;
		}
		__fallthrough;
	case SCAN_ACTION_START:
		...
		if (err) {
			LOG_DBG("Could not start scanner.");
			break;
		}
	...
	k_mutex_unlock(&scan_update_mutex);

	return err;
}

static uint32_t scan_check_if_state_allowed(enum bt_le_scan_enabled_flag flag)
{
			       ...
			       const bt_addr_le_t *addr, uint8_t adv_props)
{
	...
	bt_le_scan_user_remove(BT_LE_SCAN_USER_SCAN_BEFORE_INITIATE);
...
failed:
	...
	bt_conn_unref(conn);
	bt_le_scan_user_remove(BT_LE_SCAN_USER_NONE);
}
...
void bt_hci_le_scan_timeout(struct net_buf *buf)
{
	...
	bt_le_scan_user_remove(BT_LE_SCAN_USER_EXPLICIT_SCAN);
...
static void bt_hci_le_per_adv_sync_established_common(struct net_buf *buf)
{
	...
	if (pending_per_adv_sync) {
		...
		err = bt_le_scan_user_remove(BT_LE_SCAN_USER_SYNC_SYNCING);
...
int bt_le_scan_start(const struct bt_le_scan_param *param, bt_le_scan_cb_t cb)
{
	 ...
	 */
	k_mutex_lock(&scan_explicit_params_mutex, K_FOREVER);
	int err = scan_check_if_state_allowed(BT_LE_SCAN_USER_EXPLICIT_SCAN);
	...
	k_mutex_unlock(&scan_explicit_params_mutex);
			      ...
			      struct bt_le_per_adv_sync **out_sync)
{
	 ...
	 */
	err = bt_le_scan_user_add(BT_LE_SCAN_USER_SYNC_SYNCING);
	if (err) {
	...
	struct bt_le_per_adv_sync *per_adv_sync)
{
	...
	bt_le_scan_user_remove(BT_LE_SCAN_USER_SYNC_SYNCING);
