...
int bt_conn_disconnect(struct bt_conn *conn, uint8_t reason)
{
	...
	case BT_CONN_SCAN_BEFORE_INITIATING:
		...
		bt_conn_set_state(conn, BT_CONN_DISCONNECTED);
		if (IS_ENABLED(CONFIG_BT_CENTRAL)) {
			bt_le_scan_user_add(BT_LE_SCAN_USER_SCAN_BEFORE_INITIATE);
		}
		return 0;
	case BT_CONN_INITIATING:
		if (conn->type == BT_CONN_TYPE_LE) {
		      ...
		      const struct bt_le_conn_param *conn_param, struct bt_conn **ret_conn)
{
	...
	if (bt_dev.le.rl_entries > bt_dev.le.rl_size) {
		...
		err = bt_le_scan_user_add(BT_LE_SCAN_USER_SCAN_BEFORE_INITIATE);
		if (err) {
			bt_conn_set_state(conn, BT_CONN_DISCONNECTED);
			bt_conn_unref(conn);
	...
	if (err) {
		...
		bt_le_scan_user_add(BT_LE_SCAN_USER_NONE);
		return err;
	}
