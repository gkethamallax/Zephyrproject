			 ...
			 struct net_buf **rsp)
{
	...
	if (status) {
		LOG_WRN("opcode 0x%04x status %s", opcode,
...
static void hci_disconn_complete_prio(struct net_buf *buf)
{
	...
	LOG_DBG("status %s handle %u reason 0x%02x",
...
void bt_hci_le_enh_conn_complete(struct bt_hci_evt_le_enh_conn_complete *evt)
{
	...
	LOG_DBG("status %s handle %u role %u peer %s peer RPA %s",
...
static void le_legacy_conn_complete(struct net_buf *buf)
{
	...
	LOG_DBG("status %s role %u %s",
		bt_hci_err_to_str(evt->status), evt->role,
...
static void le_phy_update_complete(struct net_buf *buf)
{
	...
	LOG_DBG("PHY updated: status: %s, tx: %u, rx: %u",
		bt_hci_err_to_str(evt->status), evt->tx_phy,
		evt->rx_phy);
...
static void hci_encrypt_change(struct net_buf *buf)
{
	...
	LOG_DBG("status %s handle %u encrypt 0x%02x",
...
static void hci_reset_complete(struct net_buf *buf)
{
	...
	LOG_DBG("status %s", bt_hci_err_to_str(status));
...
static void hci_cmd_done(uint16_t opcode, uint8_t status, struct net_buf *evt_buf)
{
	...
	LOG_DBG("opcode 0x%04x status %s buf %p", opcode,
		bt_hci_err_to_str(status), evt_buf);
...
static void hci_cmd_complete(struct net_buf *buf)
{
	 ...
	 */
	if (opcode == BT_HCI_OP_HOST_NUM_COMPLETED_PACKETS) {
		LOG_WRN("Unexpected HOST_NUM_COMPLETED_PACKETS (status %s)",
			bt_hci_err_to_str(status));
		return;
	}
...
static void read_local_ver_complete(struct net_buf *buf)
{
	...
	LOG_DBG("status %s", bt_hci_err_to_str(rp->status));
