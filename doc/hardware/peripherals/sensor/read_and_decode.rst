...
a provided buffer by the caller. Decode (:c:func:`sensor_decode`) then
...
* Starting a read for sensors directly from an interrupt handler (no dedicated
  thread needed!)
* Starting a read for multiple sensors simultaneously
...
Additionally other shortcomings with :ref:`sensor-fetch-and-get`
related to memory and trigger handling are solved.

* Triggers result in enqueued events not callbacks
* Triggers can be setup to automatically fetch data
...
Polling a temperature sensor and printing its reading out is likely the simplest
