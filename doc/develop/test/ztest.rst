...
* :c:macro:`ZTEST_PARAM` ``(suite_name, test_name, params)``` - Which can be used to add a test by ``test_name``
  to a given suite by ``suite_name`` with an additional parameters by ``params``.
